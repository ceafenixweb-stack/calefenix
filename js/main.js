...
function startQuiz() {
  quizState.questions = shuffle(PREGUNTAS).slice(0, TOTAL_QUESTIONS);
  quizState.current = 0;
  quizState.answers = [];
  quizState.correct = 0;
...
function timeOut() {
  ...
  disableOptions();
  showFeedback(false, `⏱ Tiempo agotado. La respuesta correcta era: "${q.opts[q.ans]}"`, q.exp);
